...
extension Chart {
    ...
    public init(data: Data, sourceRange: ClosedRange<Element>? = nil, presentationRange: ClosedRange<Element>? = nil, @ViewBuilder _ content: @escaping (ChartProxy<Data, Element>) -> Content) where Data.Element == ClosedRange<Element>, Element: BinaryInteger {
...
extension ChartProxy where Data: RandomAccessCollection, Data.Element == ClosedRange<Element>, Element: BinaryInteger {
    ...
    public func presentationRange(for value: Data.Element) -> Data.Element {
        ...
        let range = (Double(value.lowerBound)...Double(value.upperBound)).map(from: source, to: destination)
        return Element(range.lowerBound)...Element(range.upperBound)
    }

    public func unitRange(for value: Data.Element) -> ClosedRange<CGFloat> {
        let source = Double(sourceRange.lowerBound)...Double(sourceRange.upperBound)
        let range = (Double(value.lowerBound)...Double(value.upperBound)).map(from: source, to: 0...1)
